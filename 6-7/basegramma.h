...
public:
 ...
 bool   terminal(tSymb code) const {return (code < (tSymb)Start);}
 std::string decode(tSymb code) const {return  Abc[code];}
 tSymb encode(const std::string& s) const {
    tInverseABC::const_iterator p = Iabc.find(s);
    return p==Iabc.end() ? 0 : p->second;
  }
 size_t altCount(tSymb left_part) const {return Prod[left_part - Start].size();}

 const tSymbstr& rightPart(tSymb left_part, tAltind alt_index)const
